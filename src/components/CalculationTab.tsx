...
import { Formula, FormulaGroup, SubFormula, ExtendedFormula } from '@/lib/types';
import { FormulaParser } from '@/lib/parser';
import { VariableMapper } from '@/lib/mapper';
import { FormulaCalculator } from '@/lib/calculator';
import { findFormulaById, getFormulaExpression } from '@/lib/formulaUtils';
...
export function CalculationTab({ editFormula, groups, mapping, ast }: CalculationTabProps) {
  ...
  const handleCalculate = useCallback(() => {
    ...
    try {
      ...
      for (const [bindingKey, formulaValues] of Object.entries(boundFormulaValues)) {
        // bindingKey 格式: "variable::refFormulaId" (使用 :: 作为分隔符，避免公式ID中包含 - 导致解析错误)
  ...
  }, [ast, variableValues, boundFormulaValues, groups, editFormula.subFormulas]);
  ...
  // 使用 useMemo 优化绑定公式变量显示的渲染
  const bindingSections = useMemo(() => {
    if (!editFormula.variableFormulaMapping || Object.keys(editFormula.variableFormulaMapping).length === 0) {
      return null;
    }
    ...
    const variableBindings: Array<{
      variable: string;
      refFormulaId: string;
      formulaInfo: { name: string; formula: string };
    ...
    for (const [variable, refFormulaId] of Object.entries(editFormula.variableFormulaMapping || {})) {
      ...
      if (lookupResult.formula) {
        try {
          ...
          // 尝试找到中文映射
          if ('chineseFormula' in lookupResult.formula && lookupResult.formula.chineseFormula) {
            try {
              const mappingResult = VariableMapper.createMapping(
                lookupResult.formula.englishFormula, 
                lookupResult.formula.chineseFormula
              );
              variables.forEach(v => {
                ...
                <div className="mb-4">
                  ...
                  <pre className="bg-white border border-gray-200 p-4 rounded-lg text-base font-mono overflow-x-auto whitespace-pre-wrap break-all text-gray-800">
                    {(() => {
                      ...
                      // 2. 处理绑定公式的值，并构建表达式
                      for (const [bindingKey, formulaValues] of Object.entries(boundFormulaValues)) {
                        // bindingKey 格式: "variable::refFormulaId" (使用 :: 作为分隔符)
                        const separatorIndex = bindingKey.indexOf('::');
                        if (separatorIndex === -1) {
                          continue;
                        }
                        ...
                        // 找到绑定公式的原始表达式
                        const formulaContent = getFormulaExpression(refFormulaId, groups, editFormula.subFormulas);
