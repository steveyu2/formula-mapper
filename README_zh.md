...
### 🧮 公式计算
- **交互式计算 Tab**: 独立的计算 Tab，支持变量输入和实时计算
- **公式绑定支持**: 可将变量绑定到其他公式或子公式，支持复杂计算
- **实时计算**: 输入变量值后自动计算结果
- **计算过程展示**: 显示完整的变量替换和公式展开过程
- **多级公式解析**: 自动计算绑定公式，并将结果用于主公式计算
...
### 公式计算
1. 在公式编辑弹窗中切换到 **计算** Tab
2. 查看主公式变量列表，输入变量值
3. 如果变量绑定了公式，会在下方显示绑定公式的输入框
4. 输入值后自动计算，显示：
   - 计算过程：完整的变量替换和公式展开
   - 最终结果：计算公式的计算结果
5. 支持多级公式绑定，自动递归计算
...
│   │   ├── CalculationTab.tsx          # 公式计算 Tab 组件
...
│       ├── parser.ts                   # 公式解析器（分词器 + AST 构建器）
│       ├── calculator.ts               # 公式计算器（计算引擎）
│       ├── mapper.ts                   # 变量映射器
