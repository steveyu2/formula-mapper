...
## 更新摘要
**所做更改**
- 新增版本管理API接口章节，包括/getVersions和/load-version端点
- 更新CloudStorageProvider接口，新增loadVersion()和getVersions()方法
- 补充Cloudflare Workers后端服务的版本管理实现
- 新增版本历史管理功能说明和使用指南
- 更新架构图展示版本管理集成方案
...
6. [本地存储API](#本地存储api)
7. [云存储API](#云存储api)
8. [Cloudflare KV存储提供者](#cloudflare-kv存储提供者)
9. [Cloudflare Workers后端服务](#cloudflare-workers后端服务)
10. [版本历史管理](#版本历史管理)
11. [依赖分析](#依赖分析)
12. [性能考虑](#性能考虑)
13. [故障排除指南](#故障排除指南)
...
**云存储API**（新增版本管理功能）：
- CloudStorageProvider接口：统一的云存储抽象层，包含版本管理方法
- CloudflareStorageProvider：Cloudflare KV存储实现，支持版本历史
- StorageProviderFactory：存储提供者工厂模式
- CloudConfigManager：云存储配置管理
- 版本历史管理：自动版本控制、历史版本查看和回滚功能

同时，文档还涵盖错误处理机制、使用场景、参数说明、返回值定义以及实际使用示例。

## 项目结构
该项目采用React + Next.js前端架构，数据存储API位于src/lib目录下，包含本地存储和云存储两个层次。云存储模块位于src/lib/cloud目录，配合Cloudflare Workers后端服务实现云端数据持久化和版本管理。
...
CloudflareWorkers --> VersionManagement
LocalAPI --> Types
CloudAPI --> Types
```
...
## 架构概览
本节展示数据存储API在整个系统中的架构关系和交互流程，包括新增的版本管理功能。
...
participant VersionManagement as 版本管理
User->>Page : 加载应用
...
User->>Page : 保存到云端
Page->>CloudflareProvider : save(key, data, comment)
CloudflareProvider->>Workers : POST /save
Workers->>KV : PUT key, data
Workers->>VersionManagement : 创建版本历史
VersionManagement->>KV : PUT versions : key : versionId, data
Workers-->>CloudflareProvider : 成功响应
CloudflareProvider-->>Page : 保存成功
User->>Page : 查看版本历史
Page->>CloudflareProvider : getVersions(key)
CloudflareProvider->>Workers : GET /versions?key=key
Workers->>VersionManagement : 获取版本列表
VersionManagement->>KV : LIST versions : key : *
Workers-->>CloudflareProvider : 版本历史列表
CloudflareProvider-->>Page : 版本列表
User->>Page : 从指定版本加载
Page->>CloudflareProvider : loadVersion(key, versionId)
CloudflareProvider->>Workers : GET /load-version?key=key&versionId=versionId
Workers->>VersionManagement : 加载指定版本
VersionManagement->>KV : GET versions : key : versionId
Workers-->>CloudflareProvider : 版本数据
...
### 数据模型设计
系统使用强类型的数据结构来确保数据的一致性和完整性，包括新增的云存储数据格式和版本管理结构。
...
CloudStorageProvider --> CloudStorageData : operates on
```
...
### API使用流程图
展示典型的数据操作流程，包括本地存储、云存储和版本管理的结合使用。
...
UserAction --> |查看版本历史| ViewVersions["调用 CloudflareProvider.getVersions()"]
ViewVersions --> ShowVersionModal["显示版本历史对话框"]
UserAction --> |从指定版本加载| LoadByVersion["调用 CloudflareProvider.loadVersion()"]
LoadByVersion --> SaveLocal
...
### 错误处理机制
系统实现了多层次的错误处理机制来确保数据操作的可靠性，包括新增的云存储错误处理和版本管理错误处理。
...
MapCloudError --> CheckVersionOp{"检查版本操作？"}
CheckVersionOp --> |是| HandleVersionError["处理版本错误"]
CheckVersionOp --> |否| ReturnCloudError["返回云错误"]
HandleVersionError --> ReturnVersionError["返回版本错误"]
CompleteCloud --> End([结束])
ReturnLocalDefault --> End
ReturnCloudError --> End
ReturnVersionError --> End
```
...
CloudStorageProvider是云存储的统一抽象接口，定义了所有云存储提供者必须实现的标准方法，现已包含版本管理功能。
...
- `save()`: 保存数据到云端，支持版本备注
- `load()`: 从云端加载指定键的数据
- `loadVersion()`: 加载指定版本的历史数据（新增）
- `getVersions()`: 获取版本历史列表（新增）
- `delete()`: 删除云端数据及所有版本
- `list()`: 列出所有云端数据键
...
CloudflareStorageProvider是Cloudflare KV存储的具体实现，继承自BaseStorageProvider基类，现已包含版本管理功能。
...
### 服务架构
Cloudflare Workers后端服务提供了RESTful API接口，与Cloudflare KV存储集成，现已包含版本管理功能。

**核心端点**
- `POST /save`: 保存数据并创建版本
- `GET /load`: 加载指定键的数据
- `GET /load-version`: 加载指定版本的数据（新增）
- `GET /versions`: 获取版本历史列表（新增）
...
**请求**
```
GET /load-version?key={key}&versionId={versionId}&namespaceId={namespaceId}
Authorization: Bearer {API_KEY}
```
...
### 版本控制机制
系统实现了自动化的版本历史管理，支持数据的版本控制和回滚功能，现已优化为最多100个版本。
...
## 依赖分析
本节分析数据存储API之间的依赖关系和耦合度，包括新增的云存储模块和版本管理功能。
...
CloudflareWorkers --> VersionManagement[版本管理]
...
- **版本管理**：自动版本历史和回滚功能，最多保留100个版本
- **错误处理**：统一的错误映射和重试机制
- **扩展性**：工厂模式支持未来添加新的存储提供商
- **安全性**：支持API密钥认证和可选的命名空间隔离
- **性能优化**：版本自动清理，避免存储空间无限增长
...
通过loadData()、saveData()、createGroup()、createFormula()和generateId()这五个核心函数，以及新增的云存储API和版本管理功能，用户可以轻松地管理公式分组和公式数据，享受流畅的使用体验和可靠的数据安全保障。