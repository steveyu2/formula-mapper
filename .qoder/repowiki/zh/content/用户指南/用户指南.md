...
- 组件层包含分组列表、公式列表、公式渲染器、AST 树、引用选择器、子公式管理器、分组选择器、云端同步按钮与模态框等
...
CSB["云端同步按钮<br/>CloudSyncButton.tsx"]
CSM["云端同步模态框<br/>CloudSyncModal.tsx"]
end
subgraph "业务逻辑层"
...
**图表来源**
- [page.tsx:1-1025](file://src/app/page.tsx#L1-L1025)
...
- [CloudSyncButton.tsx:1-206](file://src/components/CloudSyncButton.tsx#L1-L206)
- [CloudSyncModal.tsx:1-390](file://src/components/CloudSyncModal.tsx#L1-L390)
...
## 核心组件
- 页面组件：负责全局状态、URL 参数同步、分组/公式 CRUD、导入导出、数据持久化，以及增强的加载体验
...
- 云端同步：支持 Cloudflare Workers 的云端数据存储与同步
...
**章节来源**
- [page.tsx:307-467](file://src/app/page.tsx#L307-L467)
...
**章节来源**
- [page.tsx:469-540](file://src/app/page.tsx#L469-L540)
- [importExport.ts:12-120](file://src/lib/importExport.ts#L12-L120)

### 六、云端同步功能
- 云端配置
  - 点击"云同步"按钮，进入云端配置界面
  - 选择 Cloudflare Workers 服务，输入 Workers URL
  - 可选：配置 API Key 和 Namespace ID
  - 点击"保存配置"完成设置
- 云端操作
  - 保存到云端：点击"保存到云端"，将当前数据上传至云端
  - 从云端加载：点击"从云端加载"，从云端下载数据并覆盖本地数据
  - 测试连接：验证云端配置的有效性
- 增强的加载体验
  - 云端操作时显示加载指示器，确保至少显示1秒
  - 操作完成后自动清理URL参数，保持URL整洁
  - 改进的错误处理，提供清晰的错误提示

**章节来源**
- [CloudSyncButton.tsx:17-103](file://src/components/CloudSyncButton.tsx#L17-L103)
- [CloudSyncModal.tsx:42-200](file://src/components/CloudSyncModal.tsx#L42-L200)
- [page.tsx:147-212](file://src/app/page.tsx#L147-L212)
...
- 数据菜单
  - 导出/导入数据，支持键盘回车快速提交
- 云端同步菜单
  - 保存到云端、从云端加载、版本历史、配置管理
  - 支持一键测试连接和配置切换

**章节来源**
- [page.tsx:573-1016](file://src/app/page.tsx#L573-L1016)
...
- 云端同步问题
  - 检查 Workers URL 是否正确，网络连接是否正常
  - 使用"测试连接"功能验证配置有效性
  - 操作完成后URL参数会被自动清理，如仍存在可手动刷新页面
...
- [CloudSyncButton.tsx:74-103](file://src/components/CloudSyncButton.tsx#L74-L103)

## 结论
本工具提供了从"分组管理"到"公式渲染与 AST 可视化"的完整工作流，结合变量映射与引用机制，能够有效提升公式管理与教学/研究效率。新增的云端同步功能进一步增强了数据安全性。建议用户遵循"先分组、后公式、再映射"的流程，并定期导出数据以保障安全。
...
**章节来源**
- [page.tsx:173-212](file://src/app/page.tsx#L173-L212)
- [CloudSyncButton.tsx:74-103](file://src/components/CloudSyncButton.tsx#L74-L103)
- [CloudSyncModal.tsx:117-138](file://src/components/CloudSyncModal.tsx#L117-L138)