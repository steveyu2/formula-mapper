...
subgraph "工具层"
ImportExport["importExport.ts<br/>导入/导出逻辑"]
...
A --> C["storage.ts"]
D["FormulaList.tsx"] --> E["parser.ts"]
D --> F["mapper.ts"]
G["GroupList.tsx"] --> H["types.ts"]
B --> H
C --> H
```
...
章节来源
- [importExport.ts:69-118](file://src/lib/importExport.ts#L69-L118)
- [page.tsx:374-420](file://src/app/page.tsx#L374-L420)
...
## 结论
本指南总结了数据导入功能的使用方法与注意事项。通过严格的结构校验与清晰的错误提示，用户可以安全地将外部数据整合到应用中。建议在导入前做好数据备份，并遵循格式规范以减少导入失败的概率。
