...
- 公式表单：名称、英文公式、中文公式、分组选择、变量引用映射、子公式管理
...
### 公式复制（批量操作）
- 说明
  - 当前实现未提供“复制公式”快捷按钮；可通过“新建公式”+“粘贴原文”方式批量复用
...
## 结论
本指南提供了从创建到可视化的完整公式管理操作路径。通过合理的变量映射与引用机制、清晰的搜索筛选与分组体系，以及本地存储与导入导出能力，用户可以高效地组织与维护公式资产。建议在日常使用中遵循变量命名与映射规范，善用子公式与引用映射，以获得更佳的可维护性与可读性。
...
```mermaid
flowchart TD
Start(["开始"]) --> Create["新建公式<br/>填写名称/英文/中文/分组"]
Create --> Validate["必填与重名校验"]
Validate --> |通过| Save["保存并持久化"]
Validate --> |失败| Fix["修正字段"]
Fix --> Validate
Save --> Edit["编辑公式<br/>修改属性/更新映射"]
Edit --> Update["更新并持久化"]
Update --> View["展开查看<br/>变量映射/公式展示/AST"]
