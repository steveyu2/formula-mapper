...
**更新** 新增了对下划线字符的LaTeX转义处理，确保包含下划线的变量名能够正确渲染
...
- **下划线字符渲染问题**
  ...
  - 参考：[ASTTree.tsx:32-34](file://src/components/ASTTree.tsx#L32-L34)、[mapper.ts:9](file://src/lib/mapper.ts#L9)
- 变量颜色异常
  - 现象：变量颜色不一致或超出颜色范围
...
## 结论
本公式渲染系统通过清晰的组件分工与解析流程，实现了从字符串公式到富文本与 LaTeX 的多形态渲染。FormulaRenderer 负责直观的变量着色与交互，ASTTree 提供严谨的 LaTeX 渲染与中英文切换能力。**新增的下划线字符转义机制**确保了包含下划线的变量名能够在LaTeX环境中正确显示。配合 FormulaList 的引用映射与 FormulaReferenceSelector 的变量选择，用户可以高效地管理与展示复杂的数学表达式。
...
- [mapper.ts:9](file://src/lib/mapper.ts#L9)
